...
数据提取自 docs/各省数据2025/陕西.md，人工核对后固化于此；md 更新后请同步本脚本。
写入 frontend/public/data/{capacity,price,energy}.json（整省替换）。

用法:
    uv run python scripts/provinces/shaanxi.py
"""
import json
...
# ===== 装机（2025 年度，MW；来源：https://xbj.nea.gov.cn/dtyw/hyxx/202601/t20260119_294376.htm）=====
CAPACITY = dict(year=2025, thermal_mw=62000, hydro_mw=5000, wind_mw=17000, pv_mw=42000, nuclear_mw=0, other_mw=1000, total_mw=127000, source_url='https://xbj.nea.gov.cn/dtyw/hyxx/202601/t20260119_294376.html')

# ===== 电价（2025 年，元/MWh；None=未披露。行：(月, 现货, 中长期, 最高, 最低)。来源：https://www.cpem.org.cn/list2/105644.html）=====
PRICES = [
    (1, None, 359.76, None, None),
    (8, None, 380.0, None, None),
    (10, None, 353.22, None, None),]

# ===== 电量（GWh；None=未披露。行：(月, 发电量, 用电量, 跨省受入, 跨省送出)。
# 仅全年用电量估算值已披露（272,160 GWh，按可再生能源占比推算）；逐月与全年发电量未公开。来源：陕西.md §2）=====
ENERGY = [
    (0, None, 272160, None, 110000),  # 年度（送出=全年外送，md §4；用电为估算值）
]
...
def main() -> None:
    ...
    for month, spot, mlt, high, low in PRICES:
        ...
        price_items.append(dict(province_code=CODE, province_name=NAME, year=2025, month=month,
                                ...
                                source_url='https://www.cpem.org.cn/list2/105644.html'))
