...
数据提取自 docs/各省数据2025/宁夏.md，人工核对后固化于此；md 更新后请同步本脚本。
写入 frontend/public/data/{capacity,price,energy}.json（整省替换）。

用法:
    uv run python scripts/provinces/ningxia.py
"""
import json
...
CAPACITY = dict(year=2025, thermal_mw=29800, hydro_mw=422, wind_mw=22000, pv_mw=33000, nuclear_mw=0, other_mw=7630, total_mw=92852, source_url=None)
...
ENERGY = [
    (0, 250158, None, None, 100000),  # 年度（送出>1000亿kWh，取下限，md §四）
]

# ===== 年度发电结构（GWh：火/水/风/光/核；None=未提供）=====
GEN_STRUCTURE = None
PRICE_LIMITS = {"default": {"spot": {"floor": 0, "cap": 560}}}  # 宁夏.md

ANNUAL_SPOT = None  # 官方年度均价 (值, 样本月数)，如 (277.56, 4)；None=未披露
ANNUAL_MLT = None
BENCHMARK = None  # 燃煤基准价（元/MWh）
EXTRA_STATS = [{"label": "全年外送电量", "value": ">100,000 GWh（超 1000 亿千瓦时）"}, {"label": "总外送容量", "value": "20,000 MW"}]  # 宁夏.md §四
