...
数据提取自 docs/各省数据2025/贵州.md，人工核对后固化于此；md 更新后请同步本脚本。
写入 frontend/public/data/{capacity,price,energy}.json（整省替换）。

用法:
    uv run python scripts/provinces/guizhou.py
"""
import json
...
CAPACITY = dict(year=2025, thermal_mw=39481, hydro_mw=22980, wind_mw=8987, pv_mw=26495, nuclear_mw=0, other_mw=2445, total_mw=100388, source_url=None)

# ===== 电价（2025 年，元/MWh；None=未披露。行：(月, 现货, 中长期, 最高, 最低)。来源：None）=====
PRICES = [
    (1, None, 401.0, None, None),
    (2, None, 398.6, None, None),
    (3, None, 396.6, None, None),
    (4, None, 397.7, None, None),]

# ===== 电量（GWh；None=未披露。行：(月, 发电量, 用电量, 跨省受入, 跨省送出)。来源：https://www.stats.gov.cn/sj/zxfb/）=====
ENERGY = [
    (0, 256710, None, 40230, 58300),  # 年度（送出=1-10月累计，md §四）
    (3, 21560, None, None, None),
    (4, 19000, None, None, None),
    (5, 19860, None, None, None),
    (6, 20540, None, None, None),
    (7, 22720, None, None, None),
    (8, 23840, None, None, None),
    (9, 20200, None, None, None),
    (10, None, None, 5600, None),
    (12, 23000, None, None, None),]
...
EXTRA_STATS = [{"label": "1-10月送出电量", "value": "58,300 GWh（同比 -5.6%）"}]  # 贵州.md §四
