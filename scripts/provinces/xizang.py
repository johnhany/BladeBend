...
数据提取自 docs/各省数据2025/西藏.md，人工核对后固化于此；md 更新后请同步本脚本。
写入 frontend/public/data/{capacity,price,energy}.json（整省替换）。

用法:
    uv run python scripts/provinces/xizang.py
"""
import json
...
# ===== 装机（2025 年度，MW，建成口径。来源：西藏.md §一 各能源按占比估算行，合计=分项和 13,000）=====
CAPACITY = dict(year=2025, thermal_mw=1120, hydro_mw=6980, wind_mw=180, pv_mw=4720,
                nuclear_mw=0, other_mw=0, total_mw=13000,
                source_url='https://www.xizang.gov.cn')
...
ENERGY = [
    (0, None, None, None, 192500),  # 年度（送出=全年净输出，md §四）
    (5, 1210, None, None, None),
    (7, 1400, None, None, None),
    (10, 1630, None, None, None),]
