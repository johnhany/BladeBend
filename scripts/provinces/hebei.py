...
数据提取自 docs/各省数据2025/河北.md，人工核对后固化于此；md 更新后请同步本脚本。
写入 frontend/public/data/{capacity,price,energy}.json（整省替换）。

用法:
    uv run python scripts/provinces/hebei.py
"""
import json
...
# ===== 装机（2025 年度，MW；来源：https://www.cpnn.com.cn/news/dfny/202512/t20251229_1856840.h）=====
CAPACITY = dict(year=2025, thermal_mw=54300, hydro_mw=5000, wind_mw=60000, pv_mw=70000, nuclear_mw=0, other_mw=4000, total_mw=193300, source_url='https://www.cpnn.com.cn/news/dfny/202512/t20251229_1856840.html')

# ===== 电价（2025 年，元/MWh；None=未披露。行：(月, 现货, 中长期, 最高, 最低)。来源：https://pdf.dfcfw.com/pdf/H301_AP202512051794304400_1.pdf）=====
PRICES = [
    (1, None, 415.27, None, None),
    (2, None, 415.27, None, None),
    (3, 347.52, 415.27, None, None),
    (4, 368.43, 415.27, None, None),
    (5, 367.03, 415.27, None, None),
    (6, 264.46, 415.27, None, None),
    (7, 456.84, 415.27, None, None),
    (8, 433.56, 415.27, None, None),
    (9, 388.82, 415.27, None, None),
    (10, 418.55, 415.27, None, None),
    (11, None, 415.27, None, None),
    (12, None, 415.27, None, None),]

# ===== 电量（GWh；None=未披露。行：(月, 发电量, 用电量, 跨省受入, 跨省送出)。来源：https://www.chyxx.com/shuju/1250357.html）=====
ENERGY = [
    (0, 411460, 550000, None, None),  # 年度
    (12, 39400, None, None, None),]

# ===== 年度发电结构（GWh：火/水/风/光/核；None=未提供）=====
GEN_STRUCTURE = (281510, 7080, 78530, 44345, None)
PRICE_LIMITS = None  # 市场限价（元/MWh）：{月: {"spot"/"mlt": {"floor","cap"}}}；未配置则不判定异常
ANNUAL_SPOT = None  # 官方年度均价 (值, 样本月数)，如 (277.56, 4)；None=未披露
ANNUAL_MLT = None
BENCHMARK = 364.4  # 燃煤基准价（元/MWh；河北.md §三 电价表列）
...
def main() -> None:
    ...
    for month, spot, mlt, high, low in PRICES:
        ...
        price_items.append(dict(province_code=CODE, province_name=NAME, year=2025, month=month,
                                ...
                                source_url='https://pdf.dfcfw.com/pdf/H301_AP202512051794304400_1.pdf'))
    ...
    for month, gen, cons, recv, sent in ENERGY:
        it = dict(province_code=CODE, province_name=NAME, year=2025, month=month,
                  ...
                  source_url='https://www.chyxx.com/shuju/1250357.html')
