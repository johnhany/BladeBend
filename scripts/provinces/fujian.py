...
数据提取自 docs/各省数据2025/福建.md，人工核对后固化于此；md 更新后请同步本脚本。
写入 frontend/public/data/{capacity,price,energy}.json（整省替换）。

用法:
    uv run python scripts/provinces/fujian.py
"""
import json
...
CAPACITY = dict(year=2025, thermal_mw=40257, hydro_mw=17207, wind_mw=8040, pv_mw=16121, nuclear_mw=12874, other_mw=0, total_mw=94499, source_url=None)
...
ENERGY = [
    (0, 344130, 348780, None, None),  # 年度
    (3, None, 27860, None, None),
    (4, None, 28740, None, None),
    (5, None, 29050, None, None),
    (6, None, 30850, None, None),
    (7, None, 32210, None, None),
    (8, 36415, 35003, None, None),
    (9, None, 29031, None, None),
    (10, None, 27956, None, None),
    (11, None, 27644, None, None),
    (12, 29630, 22781, None, None),]

# ===== 年度发电结构（GWh：火/水/风/光/核；None=未提供）=====
GEN_STRUCTURE = (195890, 20780, 24400, 2475, 100580)
PRICE_LIMITS = None  # 市场限价（元/MWh）：{月: {"spot"/"mlt": {"floor","cap"}}}；未配置则不判定异常
ANNUAL_SPOT = None  # 官方年度均价 (值, 样本月数)，如 (277.56, 4)；None=未披露
ANNUAL_MLT = (410.0, 6)  # 月度集中竞价 1-6月累计均价，福建.md §三
BENCHMARK = 393.2  # 燃煤基准价（元/MWh；福建.md §三）
