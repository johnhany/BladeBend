...
数据提取自 docs/各省数据2025/辽宁.md，人工核对后固化于此；md 更新后请同步本脚本。
写入 frontend/public/data/{capacity,price,energy}.json（整省替换）。

用法:
    uv run python scripts/provinces/liaoning.py
"""
import json
...
# ===== 装机（2025 年度，MW；来源：https://gxt.ln.gov.cn/gxt/tztg/2025122614482656448/index.sht）=====
CAPACITY = dict(year=2025, thermal_mw=40000, hydro_mw=5000, wind_mw=22000, pv_mw=15000, nuclear_mw=7000, other_mw=0, total_mw=89000, source_url='https://gxt.ln.gov.cn/gxt/tztg/2025122614482656448/index.shtml')
...
# ===== 电量（GWh；None=未披露。行：(月, 发电量, 用电量, 跨省受入, 跨省送出)。来源：https://tjj.ln.gov.cn/tjj/tjsj/sjfb/ydsj/2025111912505720843/index.shtml）=====
ENERGY = [
    (0, 2340, 2900, 54500, 6940),  # 年度（受入=联络线全年预计545亿kWh；送出=全年跨区外送，md §四）
    (3, 205, None, None, None),
    (4, 185, None, None, None),
    (5, 181, None, None, None),
    (6, 177, 250, None, None),
    (8, 200, None, None, None),
    (12, 212, None, None, None),]
...
ANNUAL_MLT = (386.43, 12)  # 省内中长期全年平均，辽宁.md §三
...
def main() -> None:
    ...
    for month, gen, cons, recv, sent in ENERGY:
        it = dict(province_code=CODE, province_name=NAME, year=2025, month=month,
                  ...
                  source_url='https://tjj.ln.gov.cn/tjj/tjsj/sjfb/ydsj/2025111912505720843/index.shtml')
