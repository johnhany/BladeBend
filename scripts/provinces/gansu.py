...
数据提取自 docs/各省数据2025/甘肃.md，人工核对后固化于此；md 更新后请同步本脚本。
写入 frontend/public/data/{capacity,price,energy}.json（整省替换）。

用法:
    uv run python scripts/provinces/gansu.py
"""
import json
...
# ===== 装机（2025 年度，MW；来源：http://gxt.gansu.gov.cn`）=====
CAPACITY = dict(year=2025, thermal_mw=34260, hydro_mw=9720, wind_mw=39290, pv_mw=38340, nuclear_mw=0, other_mw=0, total_mw=121610, source_url='http://gxt.gansu.gov.cn`')
...
ENERGY = [
    ...
    (1, 22600, 14568, None, None),
    (4, 19660, 13725, None, None),
    (5, 20590, 14250, None, None),
    (6, 20140, 14380, None, None),
    (7, 22290, 16340, None, None),
    (8, 21580, 15450, None, None),
    (9, 19680, 14560, None, None),
    (10, 20540, 14820, None, None),
    (11, 20380, 15290, None, None),
    (12, 22800, 16550, None, None),]
...
ANNUAL_MLT = (237.49, 12)  # 全年综合省内中长期均价，甘肃.md §三
BENCHMARK = None  # 燃煤基准价（元/MWh）
EXTRA_STATS = [{"label": "8月单月跨省外送", "value": "8,865 GWh"}]  # 甘肃.md §四（仅该月披露）
