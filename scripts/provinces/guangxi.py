...
数据提取自 docs/各省数据2025/广西.md，人工核对后固化于此；md 更新后请同步本脚本。
写入 frontend/public/data/{capacity,price,energy}.json（整省替换）。

用法:
    uv run python scripts/provinces/guangxi.py
"""
import json
...
# ===== 装机（2025 年度，MW；来源：http://fgw.gxzf.gov.cn/xwzx/fgdt/t19945788.shtml）=====
CAPACITY = dict(year=2025, thermal_mw=28401, hydro_mw=19000, wind_mw=23210, pv_mw=29360, nuclear_mw=4532, other_mw=0, total_mw=104503, source_url='http://fgw.gxzf.gov.cn/xwzx/fgdt/t19945788.shtml')

# ===== 电价（2025 年，元/MWh；None=未披露。行：(月, 现货, 中长期, 最高, 最低)。来源：https://www.cwc.net.cn/mobile/news/show-36874.html）=====
PRICES = [
    (12, None, 322.411, None, None),]

# ===== 电量（GWh；None=未披露。行：(月, 发电量, 用电量, 跨省受入, 跨省送出)。来源：None）=====
ENERGY = [
    (0, None, None, None, None),  # 年度占位（全年值未披露；承载基准价/年度统计）
    (3, 20930, None, None, None),
    (4, 19500, None, None, None),
    (5, 20160, None, None, None),
    (6, 20380, None, None, None),
    (7, 22620, None, None, None),
    (11, 21010, None, None, None),
    (12, 20900, None, None, None),]
...
ANNUAL_MLT = None
BENCHMARK = 420.7  # 燃煤基准价（元/MWh；广西.md §三）
EXTRA_STATS = [{"label": "年度长协均价", "value": "341.39 元/MWh（较基准下浮 18.85%）"}]  # 广西.md §三
...
def main() -> None:
    ...
    for month, spot, mlt, high, low in PRICES:
        ...
        price_items.append(dict(province_code=CODE, province_name=NAME, year=2025, month=month,
                                ...
                                source_url='https://www.cwc.net.cn/mobile/news/show-36874.html'))
