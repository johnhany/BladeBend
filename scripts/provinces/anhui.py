...
数据提取自 docs/各省数据2025/安徽.md，人工核对后固化于此；md 更新后请同步本脚本。
写入 frontend/public/data/{capacity,price,energy}.json（整省替换）。

用法:
    uv run python scripts/provinces/anhui.py
"""
import json
...
CAPACITY = dict(year=2025, thermal_mw=0, hydro_mw=6000, wind_mw=10500, pv_mw=55000, nuclear_mw=0, other_mw=8000, total_mw=79500, source_url=None)

# ===== 电价（2025 年，元/MWh；None=未披露。行：(月, 现货, 中长期, 最高, 最低)。
# 现货=日前均价（1-6月）；中长期=月度直接交易均价（1-5月）。来源：安徽.md §三 两张表）=====
PRICES = [
    (1, 278.06, 406.34, None, None),
    (2, 267.99, 402.55, None, None),
    (3, 274.13, 405.39, None, None),
    (4, 252.63, 406.69, None, None),
    (5, 276.36, 406.88, None, None),
    (6, 196.37, None, None, None),
]

# ===== 电量（GWh；None=未披露。行：(月, 发电量, 用电量, 跨省受入, 跨省送出)。来源：None）=====
ENERGY = [
    (1, 29310, 29950, None, None),
    (2, 26725, 28900, None, None),
    (3, 29200, 31600, None, None),
    (4, 27200, 29200, None, None),
    (5, 28800, 171770, None, None),
    (6, 30500, None, None, None),
    (7, 41170, 252780, None, None),
    (8, 39840, None, None, None),
    (9, 31700, 33800, None, None),
    (10, 29340, 32500, None, None),
    (11, 28440, 342240, None, None),
    (12, None, 35226, None, None),]
