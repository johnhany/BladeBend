...
数据提取自 docs/各省数据2025/湖南.md，人工核对后固化于此；md 更新后请同步本脚本。
写入 frontend/public/data/{capacity,price,energy}.json（整省替换）。

用法:
    uv run python scripts/provinces/hunan.py
"""
import json
...
# ===== 装机（2025 年度，MW；来源：https://www.cpnn.com.cn/news/nyqy/202602/t20260201_1864060.h）=====
CAPACITY = dict(year=2025, thermal_mw=32586, hydro_mw=17890, wind_mw=22350, pv_mw=20000, nuclear_mw=0, other_mw=6380, total_mw=99206, source_url='https://www.cpnn.com.cn/news/nyqy/202602/t20260201_1864060.html')
...
# ===== 电量（GWh；None=未披露。行：(月, 发电量, 用电量, 跨省受入, 跨省送出)。来源：https://www.cir.cn/R_NengYuanKuangChan/2026-01/2025Nian9YueHuNanShengFaDianLiangTongJiFenXi.html）=====
ENERGY = [
    (0, None, 245500, 60382, None),  # 年度（受入=全年累计受入电量，md §4.1）
    (3, 14000, None, None, None),
    (4, 12260, None, None, None),
    (5, 13130, 17311, None, None),
    (6, 14520, None, None, None),
    (7, 17090, None, None, None),
    (8, 16280, None, None, None),
    (9, 13400, None, None, None),
    (10, 14840, None, None, None),
    (11, 11300, None, None, None),
    (12, 12040, None, None, None),]

# ===== 年度发电结构（GWh：火/水/风/光/核；None=未提供）=====
GEN_STRUCTURE = None
PRICE_LIMITS = {"default": {"spot": {"floor": 0, "cap": 900}}}  # 湖南.md

ANNUAL_SPOT = None  # 官方年度均价 (值, 样本月数)，如 (277.56, 4)；None=未披露
ANNUAL_MLT = (452.461, 12)  # 发电企业年度结算均价，湖南.md §三
...
def main() -> None:
    ...
    for month, gen, cons, recv, sent in ENERGY:
        it = dict(province_code=CODE, province_name=NAME, year=2025, month=month,
                  ...
                  source_url='https://www.cir.cn/R_NengYuanKuangChan/2026-01/2025Nian9YueHuNanShengFaDianLiangTongJiFenXi.html')
