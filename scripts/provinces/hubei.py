...
数据提取自 docs/各省数据2025/湖北.md，人工核对后固化于此；md 更新后请同步本脚本。
写入 frontend/public/data/{capacity,price,energy}.json（整省替换）。

用法:
    uv run python scripts/provinces/hubei.py
"""
import json
...
# ===== 装机（2025 年度，MW；来源：https://vip.stock.finance.sina.com.cn/corp/view/vCB_AllBulle）=====
CAPACITY = dict(year=2025, thermal_mw=41188, hydro_mw=38134, wind_mw=10387, pv_mw=42828, nuclear_mw=0, other_mw=0, total_mw=132537, source_url='https://vip.stock.finance.sina.com.cn/corp/view/vCB_AllBulletinDetail.php?stockid=000883&id=11409208')
...
# ===== 电量（GWh；None=未披露。行：(月, 发电量, 用电量, 跨省受入, 跨省送出)。来源：https://fgw.hubei.gov.cn/fbjd/xxgkml/jgzn/wgdw/nyj/dlddc/tzgg/202502/t20250214_5542006.shtml）=====
ENERGY = [
    (0, 311270, 262902, None, None),  # 年度
    (1, 23387, 19763, None, None),
    (2, 21288, 18096, None, None),
    (3, 23854, 20586, None, None),
    (4, 22519, 21057, None, None),
    (5, 24275, 21365, None, None),
    (6, 25558, 21394, None, None),
    (7, 35573, 29665, None, None),
    (8, 34660, 29116, None, None),
    (9, 26702, 20970, None, None),
    (10, 24489, 20880, None, None),
    (11, 23446, 19596, None, None),
    (12, 25519, 20414, None, None),]
...
ANNUAL_MLT = (437.448, 12)  # 全年批发侧结算均价，湖北.md §三
...
def main() -> None:
    ...
    for month, gen, cons, recv, sent in ENERGY:
        it = dict(province_code=CODE, province_name=NAME, year=2025, month=month,
                  ...
                  source_url='https://fgw.hubei.gov.cn/fbjd/xxgkml/jgzn/wgdw/nyj/dlddc/tzgg/202502/t20250214_5542006.shtml')
